<div class="container p-3 card">
			...
			<div class="taxes">
				@if ($voucher->vat > 0)
					<tr>
						...
						<td>VAT</td>
						<td>7.5%</td>
						<td>{{ Str::currency($voucher->vat) }}</td>
					</tr>
				@endif
				@if ($voucher->wht > 0)
					<tr>
						<td></td>
						<td>WHT</td>
						<td>{{ $voucher->type == 'Contract' ? '5%' : '10%' }}</td>
						<td>{{ Str::currency($voucher->wht) }}</td>
					</tr>
				@endif
				@if ($voucher->stamp_duty > 0)
					<tr>
						<td></td>
						<td>Stamp Duty</td>
						<td>1%</td>
						<td>{{ Str::currency($voucher->stamp_duty) }}</td>
					</tr>
				@endif
			</div>
			<tr>
				<td colspan="2"> Total Amount in Words : {{ Str::inwords($voucher->amount) }}</td>
				<td></td>
				<td></td>
			</tr>
		</table>
	...
	<div class="row">
		...
		<div class="col-6 text-center">
			...
			Date : {{ Str::dayDate($voucher->checked_date) }}
		</div>
	</div>
	<div class="row">
	...
	<div class="row text-center justify-content-center">
		@if ($voucher->audited_by != null)
			<!-- (C) WATERMARK -->
			<div id="watermark" class="text-center text-danger">

				<h3>Voucher has been processed and cleared by audit</h3>
			</div>
		@endif
	</div>
</div>
