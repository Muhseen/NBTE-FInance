...
@section('content')
	<div class="container-fluid card p-2">
		...
		<div class="row justify-content-end">
			...
			<form action="/recheck/{{ $voucher->id }}" method="Get">
				<button class="btn btn-warning  mx-3">Return for Recheck</button>
			</form>
			<form action="/approveVoucher" method="POST">
				@csrf
				<input type="hidden" value="{{ $voucher->id }}" name="voucher_id">
				<button class="btn btn-success mr-3">Approve Voucher(Send to audit)</button>
			</form>
		</div>
		<div class="row mx-2">
